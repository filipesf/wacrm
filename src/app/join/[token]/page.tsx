...
// Maps the API's snake_case failure reasons to the camelCase catalogue
// keys under `auth.join.fail.*`.
  ...
  const handleAccept = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (res.status === 409) {
          setConflictMessage(
            payload.error || t('conflict.defaultMessage'),
          );
        } else {
      ...
      toast.error(t('toast.serverUnreachable'));
      setAccepting(false);
    }
  }, [token, t]);
    ...
    <CardHeader className="items-center text-center">
      ...
      <CardDescription className="text-muted-foreground">
        {t.rich('joinAs', {
          ...
          date: fmt.date(new Date(peek.expires_at), {
            year: 'numeric',
            month: 'short',
            day: 'numeric',
          }),
          ...
          <DialogContent className="bg-popover border-border sm:max-w-md">
            <DialogHeader>
              <DialogTitle className="flex items-center gap-2 text-popover-foreground">
                ...
                {t('conflict.title', { accountName: peek.account_name })}
              </DialogTitle>
            ...
            <DialogFooter className="bg-popover border-border">
              ...
              >
                {signingOut ? (
                  ...
                  t('conflict.signOutAndUseDifferentEmail')
                )}
              </Button>
            </DialogFooter>
          </DialogContent>
