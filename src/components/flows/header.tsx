  ...
  return (
    <div className="flex flex-col gap-1.5 px-6 pt-5">
        ...
        <div className="ml-auto flex flex-wrap items-center gap-1.5">
          ...
          {state.status === "active" ? (
            ...
            <Button
              ...
              title={!canActivate ? t("header.activateBlocked") : undefined}
            >
              {activating ? (
                <Loader2 className="h-3.5 w-3.5 animate-spin" />
              ) : (
      ...
      <input
        ...
        aria-label={t("header.descriptionAria")}
...
function StatusChip({ status }: { status: BuilderState["status"] }) {
  const t = useTranslations("flows");
  const cls = {
    // Neutral, not amber — amber is reserved for the adjacent "Edited"
    // dirty signal, so the two don't read as the same alert.
