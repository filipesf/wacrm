  ...
  switch (node.node_type) {
    ...
    case "send_message":
      return (
        <>
          <TextRow
            label={t("forms.sendMessage.text")}
    ...
    case "collect_input":
      return (
        <>
          <TextRow
            label={t("forms.collectInput.prompt")}
          ...
          <div>
            <label className="mb-1 block text-xs text-muted-foreground">
              {t("forms.collectInput.varKeyLabel")}
            </label>
            <Input
              ...
              placeholder={t("forms.collectInput.varKeyPlaceholder")}
              className="bg-muted font-mono text-xs"
            />
            <p className="mt-1 text-[10px] text-muted-foreground">
              {t.rich("forms.collectInput.interpolateHint", {
                code: (chunks) => (
                  <code className="rounded bg-muted px-1">{chunks}</code>
                ),
                token: `{{vars.${(cfg as { var_key?: string }).var_key || "name"}}}`,
              })}
            </p>
          </div>
          <NextNodeRow
            ...
            currentKey={node.node_key}
            onChange={(v) => onUpdateConfig({ next_node_key: v })}
            label={t("forms.collectInput.afterCapture")}
          />
        </>
      );
    ...
    case "end":
      return (
        <p className="text-xs text-muted-foreground">{t("forms.end")}</p>
      );
  }
}
...
function SendButtonsForm({
  ...
  return (
    <>
      <TextRow
        label={t("forms.buttons.body")}
        value={cfg.text ?? ""}
        onChange={(v) => onUpdateConfig({ text: v })}
        rows={3}
      />
      <TextRow
        label={t("forms.buttons.footer")}
          ...
          {buttons.map((b, i) => (
            ...
            >
              ...
              <Input
                ...
                placeholder={t("forms.buttons.titlePlaceholder")}
                className="bg-muted"
                maxLength={20}
              />
              <NodeKeySelect
                ...
                placeholder={t("forms.nextNodePlaceholder")}
              />
              <Button
                variant="ghost"
                size="sm"
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
        <TextRow
          label={t("forms.list.buttonLabel")}
          value={cfg.button_label ?? ""}
          onChange={(v) => onUpdateConfig({ button_label: v })}
        />
        <TextRow
          label={t("forms.list.footer")}
          value={cfg.footer_text ?? ""}
          onChange={(v) => onUpdateConfig({ footer_text: v })}
        />
      </div>
          ...
          >
            <div className="mb-2 flex items-center gap-2">
              <Input
                ...
                placeholder={t("forms.list.sectionTitlePlaceholder", {
                  number: sIdx + 1,
                })}
              ...
              {sections.length > 1 && (
                <Button
                  ...
                  aria-label={t("forms.list.removeSection")}
                >
                  <Trash2 className="h-3.5 w-3.5" />
                </Button>
              )}
            ...
            {section.rows.map((row, rIdx) => (
              ...
              >
                ...
                <Input
                  ...
                  placeholder={t("forms.list.rowTitlePlaceholder")}
                  className="bg-muted"
                  maxLength={24}
                />
                <NodeKeySelect
    ...
    <>
      <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
          ...
          >
            ...
            <SelectContent>
              <SelectItem value="var">
                {t("forms.condition.subjectVar")}
              </SelectItem>
              <SelectItem value="tag">
                {t("forms.condition.subjectTag")}
              </SelectItem>
              <SelectItem value="contact_field">
                {t("forms.condition.subjectField")}
              </SelectItem>
            </SelectContent>
          </Select>
        </div>
        <div className="md:col-span-2">
          <label className="mb-1 block text-xs text-muted-foreground">
            {subject === "var"
              ? t("forms.condition.varName")
              : subject === "tag"
                ? t("forms.condition.tag")
                : t("forms.condition.field")}
          </label>
          {subject === "tag" && tags.length > 0 ? (
            ...
            >
              <SelectTrigger className="bg-muted">
                <SelectValue placeholder={t("forms.condition.pickTag")} />
              </SelectTrigger>
              <SelectContent>
                {tags.map((tag) => (
            ...
            >
              <SelectTrigger className="bg-muted">
                <SelectValue placeholder={t("forms.condition.pickField")} />
              </SelectTrigger>
              <SelectContent>
            ...
            <Input
              ...
              placeholder={
                subject === "var"
                  ? t("forms.condition.varKeyPlaceholder")
                  : t("forms.condition.tagUuidPlaceholder")
              }
              className="bg-muted font-mono text-xs"
            />
          )}
          ...
          >
            ...
            <SelectContent>
              <SelectItem value="present">
                {t("forms.condition.opPresent")}
              </SelectItem>
              <SelectItem value="absent">
                {t("forms.condition.opAbsent")}
              </SelectItem>
              <SelectItem value="equals">
                {t("forms.condition.opEquals")}
              </SelectItem>
              <SelectItem value="contains">
                {t("forms.condition.opContains")}
              </SelectItem>
            </SelectContent>
          </Select>
        </div>
      ...
      <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
        <NextNodeRow
          ...
          label={t("forms.condition.trueNext")}
        />
        <NextNodeRow
          ...
          label={t("forms.condition.falseNext")}
        />
      </div>
    </>
  );
  ...
  return (
    <>
      <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
        <div>
          ...
          >
            ...
            <SelectContent>
              <SelectItem value="add">{t("forms.setTag.add")}</SelectItem>
              <SelectItem value="remove">{t("forms.setTag.remove")}</SelectItem>
            </SelectContent>
          </Select>
        </div>
        <div>
          ...
          {tags.length > 0 ? (
            ...
            >
              <SelectTrigger className="bg-muted">
                <SelectValue placeholder={t("forms.setTag.pickTag")} />
              </SelectTrigger>
              <SelectContent>
                {tags.map((tag) => (
                  <SelectItem key={tag.id} value={tag.id}>
            ...
            <Input
              ...
              placeholder={t("forms.setTag.tagUuidPlaceholder")}
              className="bg-muted font-mono text-xs"
            />
          )}
        </div>
      ...
      <NextNodeRow
        ...
        currentKey={currentKey}
        onChange={(v) => onUpdateConfig({ next_node_key: v })}
        label={t("forms.setTag.thenAdvance")}
      />
    </>
  );
}
  ...
  const handleFile = useCallback(
    async (file: File) => {
      ...
      try {
        ...
        toast.success(t("forms.media.uploaded"));
      } catch (err) {
        const msg =
          err instanceof Error ? err.message : t("forms.media.uploadFailed");
        toast.error(msg);
      } finally {
        setUploading(false);
      }
    ...
    <>
      <div>
        ...
        >
          ...
          <SelectContent>
            <SelectItem value="image">{t("forms.media.image")}</SelectItem>
            <SelectItem value="video">{t("forms.media.video")}</SelectItem>
            <SelectItem value="document">
              {t("forms.media.document")}
            </SelectItem>
          </SelectContent>
        </Select>
      </div>
        ...
        {cfg.media_url ? (
          <div className="flex items-center gap-2 rounded-md border border-border bg-muted px-3 py-2 text-xs">
            ...
            <button
              ...
              aria-label={t("forms.media.removeFile")}
              disabled={uploading}
            >
          ...
          >
            {uploading ? (
              <>
                ...
                {t("forms.media.uploading")}
              </>
            ) : (
              <>
                <Upload className="h-3.5 w-3.5" />
                {t("forms.media.upload")}
              </>
            )}
          </button>
        )}
      ...
      {isDocument && (
        <div>
          <label className="mb-1 block text-xs text-muted-foreground">
            {t("forms.media.filenameLabel")}
          </label>
          <Input
            value={cfg.filename ?? ""}
            onChange={(e) => onUpdateConfig({ filename: e.target.value })}
            placeholder={t("forms.media.filenamePlaceholder")}
            className="bg-muted text-xs"
          />
        </div>
      )}
      ...
      <NextNodeRow
        ...
        currentKey={currentKey}
        onChange={(v) => onUpdateConfig({ next_node_key: v })}
        label={t("forms.media.afterSending")}
      />
    </>
  );
}
