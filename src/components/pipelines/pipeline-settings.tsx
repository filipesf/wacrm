...
export function PipelineSettings({
  ...
  async function handleSave() {
    ...
    if (renameRes.error || stagesRes.error) {
      toast.error(t("toasts.pipelineSaveFailed"));
      return;
    }
    ...
    onPipelinesChanged();
    onStagesChanged();
    toast.success(t("toasts.pipelineSaved"));
  }
  ...
  async function handleAddStage() {
    ...
    const { data, error } = await supabase
      ...
      .single();
    if (error || !data) {
      toast.error(t("toasts.stageAddFailed"));
      return;
    }
  ...
  async function handleRemoveStage(stageId: string) {
    ...
    const { count } = await supabase
      ...
      .eq("stage_id", stageId);
    if (count && count > 0) {
      toast.error(t("toasts.stageHasDeals"));
      return;
    }
    const { error } = await supabase
      ...
      .eq("id", stageId);
    if (error) {
      toast.error(t("toasts.stageDeleteFailed"));
      return;
    }
  ...
  async function handleDeletePipeline() {
    ...
    setDeleting(false);
    if (error) {
      toast.error(t("toasts.pipelineDeleteFailed"));
      return;
    }
    onOpenChange(false);
    onPipelinesChanged();
    toast.success(t("toasts.pipelineDeleted"));
  }
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md bg-popover border-border max-h-[85vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="text-popover-foreground">{t("settings.title")}</DialogTitle>
        </DialogHeader>
        ...
        {showDeleteConfirm ? (
          <div className="py-4">
            ...
            <div className="mt-4 flex justify-end gap-2">
              ...
              >
                {deleting ? t("settings.deleting") : t("settings.deletePipeline")}
              </Button>
            </div>
          </div>
        ) : (
          <>
            <div className="grid gap-4 py-2">
              <div className="grid gap-2">
                <Label className="text-muted-foreground">{t("settings.nameLabel")}</Label>
                <Input
                  value={name}
              ...
              <div className="grid gap-2">
                <Label className="text-muted-foreground">{t("settings.stagesLabel")}</Label>
                <DndContext
                  sensors={sensors}
                ...
                <div className="mt-1 flex flex-wrap gap-1">
                  {STAGE_COLORS.map((color) => (
                    <button
                      ...
                      aria-label={t("settings.pickColor", { color })}
                    />
                  ))}
                </div>
