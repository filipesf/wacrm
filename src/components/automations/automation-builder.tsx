...
// Labels live in the `automations.steps.<type>` catalog; here we keep only
// the icon + border accent, which aren't localizable.
const STEP_META: Record<AutomationStepType, StepMeta> = {
  ...
  wait: { icon: Hourglass, border: "border-l-border" },
...
// Display order for the trigger picker. Labels + hints live in the
// `automations.builder.triggerOptions.<value>` catalog.
...
function TagSelect({
  ...
  if (tags.length === 0) {
    return (
      <Input
        placeholder={t("builder.fields.tagIdPlaceholder")}
        value={value}
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      >
        <option value="">{t("builder.fields.selectTag")}</option>
        {tags.map((tag) => (
        ...
        {value && !selected && (
          <option value={value}>{t("builder.fields.unknownTag", { value })}</option>
        )}
      </select>
    </div>
  )
...
function ContactFieldSelect({
  ...
  return (
    <select
      ...
      className={SELECT_CLASS}
    >
      <option value="name">{t("builder.fields.fieldName")}</option>
      <option value="email">{t("builder.fields.fieldEmail")}</option>
      <option value="company">{t("builder.fields.fieldCompany")}</option>
      {customFields.length > 0 && (
        <optgroup label={t("builder.fields.customFields")}>
          {customFields.map((f) => (
      ...
      {customValue && !knownCustom && (
        <option value={customValue}>{t("builder.fields.unknownField", { value: customValue })}</option>
      )}
    </select>
  )
}
...
function AgentSelect({
  ...
  if (members.length === 0) {
    return (
      <Input
        placeholder={t("builder.fields.agentIdPlaceholder")}
        value={value}
        onChange={(e) => onChange(e.target.value)}
        className="bg-muted text-foreground"
      />
  ...
  return (
    <select
      ...
      className={SELECT_CLASS}
    >
      <option value="">{t("builder.fields.selectAgent")}</option>
      {members.map((m) => (
      ...
      {value && !selected && (
        <option value={value}>{t("builder.fields.unknownAgent", { value })}</option>
      )}
    </select>
  )
}
...
function DealPipelineFields({
  ...
  return (
    <>
      <FieldBlock label={t("builder.fields.pipeline")}>
        <select
          value={pipelineId}
          onChange={(e) => {
          ...
          className={SELECT_CLASS}
        >
          <option value="">{t("builder.fields.selectPipeline")}</option>
          {pipelines.map((p) => (
          ...
          {pipelineId && !selectedPipeline && (
            <option value={pipelineId}>{t("builder.fields.unknownPipeline", { value: pipelineId })}</option>
          )}
        </select>
      </FieldBlock>
      <FieldBlock label={t("builder.fields.stage")}>
        <select
          value={stageId}
          onChange={(e) =>
            onChange({ pipeline_id: pipelineId, stage_id: e.target.value })
        ...
        >
          <option value="">
            {pipelineId ? t("builder.fields.selectStage") : t("builder.fields.selectPipelineFirst")}
          </option>
          {stageOptions.map((s) => (
          ...
          {stageId && pipelineId && !selectedStage && (
            <option value={stageId}>{t("builder.fields.unknownStage", { value: stageId })}</option>
          )}
        </select>
      </FieldBlock>
    </>
...
function SendTemplateFields({
  ...
  return (
    <FieldBlock label={t("builder.fields.template")}>
      <select
        value={current}
        onChange={(e) => {
        ...
        className={SELECT_CLASS}
      >
        <option value="">{t("builder.fields.selectTemplate")}</option>
        {templates.map((tpl) => {
        ...
        {current && !hasMatch && (
          <option value={current}>
            {t("builder.fields.templateNotApproved", {
              name: templateName,
              language: language || t("builder.fields.templateUnknownLang"),
            })}
          </option>
        )}
      </select>
...
export function AutomationBuilder({ initial }: { initial: BuilderInitial }) {
  ...
  async function save() {
    ...
    try {
      const payload = {
        name: state.name || t("builder.untitled"),
        ...
        if (firstIssue?.message) {
          toast.error(firstIssue.message, {
            description: firstIssue.path
              ? t("builder.toast.issueAt", { path: firstIssue.path })
              : undefined,
          })
        } else {
          toast.error(body?.error ?? t("builder.toast.saveFailed"))
        }
        return
      }
      toast.success(isEditing ? t("builder.toast.saved") : t("builder.toast.created"))
...
function TriggerCard({
  ...
  const typeLabel = t.has(`builder.triggerOptions.${type}.label`)
    ? t(`builder.triggerOptions.${type}.label`)
    : type
  return (
      ...
      <div className="rounded-lg border border-border border-l-4 border-l-blue-500 bg-card shadow-lg">
        ...
        >
          ...
          <div className="min-w-0 flex-1">
            <div className="text-[11px] uppercase tracking-wide text-blue-300">{t("builder.trigger.eyebrow")}</div>
        ...
        {open && (
          <div className="space-y-3 border-t border-border px-4 py-3">
              ...
              >
                {TRIGGER_TYPES.map((value) => (
                  <option key={value} value={value}>
                    {t(`builder.triggerOptions.${value}.label`)}
                  </option>
                ))}
              </select>
              <p className="mt-1 text-[11px] text-muted-foreground">
                {t.has(`builder.triggerOptions.${type}.hint`)
                  ? t(`builder.triggerOptions.${type}.hint`)
                  : null}
              </p>
            </div>
            {type === "keyword_match" && (
            ...
            {type === "time_based" && (
              <Input
                placeholder={t("builder.trigger.schedulePlaceholder")}
...
function KeywordMatchConfig({
  ...
  return (
    <div className="space-y-2">
      <div>
        <label className="mb-1 block text-xs font-medium text-muted-foreground">
          {t("builder.trigger.keywordsLabel")}
        </label>
        <Input
          value={draft}
          ...
          placeholder={t("builder.trigger.keywordsPlaceholder")}
          className="bg-muted text-foreground"
        />
      </div>
      <div>
        <label className="mb-1 block text-xs font-medium text-muted-foreground">
          {t("builder.trigger.matchTypeLabel")}
        </label>
        <select
        ...
        >
          <option value="contains">{t("builder.trigger.matchType.contains")}</option>
          <option value="exact">{t("builder.trigger.matchType.exact")}</option>
        </select>
      </div>
    </div>
  )
...
function StepRenderer({
  ...
  const kind = isCondition ? "condition" : step.step_type === "wait" ? "wait" : "action"
          ...
          >
            ...
            <div className="min-w-0 flex-1">
              <div className="text-[11px] uppercase tracking-wide text-muted-foreground">
                {t(`builder.stepKinds.${kind}`)}
              </div>
              <div className="truncate text-sm font-medium text-foreground">{t(`steps.${step.step_type}`)}</div>
...
function ConditionBranches({
  ...
  return (
    ...
    <div className="mt-3 grid grid-cols-1 gap-3 sm:grid-cols-2">
      <BranchColumn label={t("builder.branch.yes")} color="text-primary">
        <StepList {...props} steps={yes} parentPath={yesPath} />
      </BranchColumn>
      <BranchColumn label={t("builder.branch.no")} color="text-rose-400">
...
function StepEditor({
  ...
  switch (step.step_type) {
      ...
      return (
        <FieldBlock label={t("builder.fields.messageText")}>
          <Textarea
            ...
            placeholder={t("builder.fields.messageTextPlaceholder")}
    ...
    case "remove_tag":
      return (
        <FieldBlock label={t("builder.fields.tag")}>
          <TagSelect
      ...
      return (
        <>
          <FieldBlock label={t("builder.fields.mode")}>
            ...
            >
              <option value="round_robin">{t("builder.fields.modeRoundRobin")}</option>
              <option value="specific">{t("builder.fields.modeSpecific")}</option>
            </select>
          </FieldBlock>
          {cfg.mode === "specific" && (
            <FieldBlock label={t("builder.fields.agent")}>
              <AgentSelect
        ...
        <>
          ...
          <FieldBlock label={t("builder.fields.value")}>
            <Input
              ...
              placeholder={t("builder.fields.valuePlaceholder", {
                sample: "{{ vars.x }} / {{ message.text }}",
              })}
              className="bg-muted text-foreground"
            />
          </FieldBlock>
        ...
        <div className="grid grid-cols-2 gap-2">
          ...
          <FieldBlock label={t("builder.fields.unit")}>
            <select
              ...
              className="w-full rounded-md border border-border bg-muted px-2 py-1.5 text-sm text-foreground"
            >
              <option value="minutes">{t("builder.units.minutes")}</option>
              <option value="hours">{t("builder.units.hours")}</option>
              <option value="days">{t("builder.units.days")}</option>
            </select>
          </FieldBlock>
        </div>
      )
    case "condition":
      return (
        <>
          <FieldBlock label={t("builder.condition.subject")}>
            <select
              value={(cfg.subject as string) ?? "tag_presence"}
              onChange={(e) => set({ subject: e.target.value })}
              className="w-full rounded-md border border-border bg-muted px-2 py-1.5 text-sm text-foreground"
            >
              <option value="tag_presence">{t("builder.condition.subjects.tag_presence")}</option>
              <option value="contact_field">{t("builder.condition.subjects.contact_field")}</option>
              <option value="message_content">{t("builder.condition.subjects.message_content")}</option>
              <option value="time_of_day">{t("builder.condition.subjects.time_of_day")}</option>
            </select>
          </FieldBlock>
          <FieldBlock label={t("builder.condition.operand")}>
            <Input
              placeholder={
                typeof cfg.subject === "string" &&
                t.has(`builder.condition.operandPlaceholder.${cfg.subject}`)
                  ? t(`builder.condition.operandPlaceholder.${cfg.subject}`)
                  : ""
              }
    ...
    case "send_webhook":
      return (
        <>
          <FieldBlock label={t("builder.fields.webhookUrl")}>
            <Input
    ...
    case "close_conversation":
      return (
        <p className="text-xs text-muted-foreground">
          {t("builder.fields.closeConversationNote")}
        </p>
      )
    default:
      return null
...
function previewFor(step: BuilderStep, t: TFn): string {
  switch (step.step_type) {
    case "send_message":
      return (step.step_config.text as string) || t("builder.preview.noText")
    case "send_template":
      return (step.step_config.template_name as string) || t("builder.preview.pickTemplate")
    case "wait":
      return t("builder.preview.wait", {
        amount: (step.step_config.amount as number | string) ?? "?",
        unit: (step.step_config.unit as string) ?? "",
      })
    case "condition":
      return t("builder.preview.condition", {
        subject: (step.step_config.subject as string) ?? "?",
      })
    case "send_webhook":
      return (step.step_config.url as string) || t("builder.preview.noUrl")
    default:
      return ""
  }
}
