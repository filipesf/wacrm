...
const EXPIRY_VALUES = ['1', '7', '30'] as const;
...
export function InviteMemberDialog({
  ...
  async function handleCreate() {
    ...
    if (trimmedLabel.length > MAX_LABEL_LEN) {
      toast.error(t('maxLabel', { max: MAX_LABEL_LEN }));
      return;
    }
    setSubmitting(true);
    try {
      ...
      setResult({
        ...
        accountName: account?.name ?? t('accountFallback'),
      });
      onCreated();
    } catch (err) {
      console.error('[InviteMemberDialog] create error:', err);
      toast.error(t('serverRetry'));
    } finally {
      setSubmitting(false);
    }
  }
  ...
  function whatsappShareUrl(url: string): string {
    ...
    const accountName = result?.accountName ?? t('accountFallback');
        ...
        {result ? (
          <>
            <DialogHeader>
              ...
              <DialogDescription className="text-muted-foreground">
                {t.rich('createdDesc', {
                  ...
                  days: () => (
                    <span className="font-medium text-muted-foreground">
                      {t('expiry', { count: result.expiresInDays })}
                    </span>
                  ),
                })}
              </DialogDescription>
            ...
            <div className="space-y-3 py-2">
              <Label className="text-muted-foreground">{t('inviteLinkLabel')}</Label>
          ...
          <>
            <DialogHeader>
              <DialogTitle className="text-popover-foreground">{t('formTitle')}</DialogTitle>
              <DialogDescription className="text-muted-foreground">
                {t('formDesc')}
              </DialogDescription>
            </DialogHeader>
