  ...
  async function handleSave() {
    ...
    try {
      ...
      if (data.registered === false && data.registration_error) {
        toast.error(
          t('registerFailed', { error: data.registration_error }),
          { duration: 12000 },
        );
        ...
        toast.success(
          data.phone_info?.verified_name
            ? t('liveNamed', { name: data.phone_info.verified_name })
            : t('connectedGeneric'),
        );
      ...
      if (payload.connected) {
        ...
        toast.success(
          payload.phone_info?.verified_name
            ? t('connectedNamed', { name: payload.phone_info.verified_name })
            : t('apiSuccess')
        );
      } else {
        setConnectionStatus('disconnected');
        setResetReason(payload.needs_reset ? 'token_corrupted' : payload.reason === 'meta_api_error' ? 'meta_api_error' : null);
        setStatusMessage(payload.message || '');
        toast.error(payload.message || t('apiFailed'));
      }
    } catch (err) {
  ...
  async function handleVerifyRegistration() {
    ...
    try {
      ...
      if (data.live) {
        ...
        toast.error(t('registrationNotFull'), { duration: 8000 });
      }
      if (accountId) await fetchConfig(accountId);
    } catch (err) {
  ...
  return (
    <section className="animate-in fade-in-50 duration-200">
      ...
      <div className="space-y-6">
        ...
        <Alert className="bg-card border-border">
          <div className="flex items-center gap-2">
            ...
            <AlertTitle className="text-foreground mb-0">
              {connectionStatus === 'connected' ? t('credsValid') : t('notConnected')}
            </AlertTitle>
          </div>
        ...
        {config && (
          ...
          >
            ...
            <AlertDescription className="text-muted-foreground mt-2 text-xs leading-relaxed">
              {isRegistered
                ? t.rich('registeredDesc', {
                    date: config.registered_at
                      ? fmt.date(new Date(config.registered_at), {
                          dateStyle: 'medium',
                          timeStyle: 'short',
                        })
                      : t('unknown'),
                ...
                : lastRegistrationError
                  ? t.rich('lastFailedDesc', {
                      error: lastRegistrationError,
                      err: (chunks) => (
                        <span className="text-red-300">{chunks}</span>
                      ),
                    })
                  : t('neverRegisteredDesc')}
            </AlertDescription>
              ...
              <div className="mt-3 rounded border border-border bg-card/60 px-3 py-2 space-y-1.5 text-[11px]">
                <p className="font-medium text-foreground">
                  ...
                  <span className={registrationProbe.live ? 'text-emerald-400' : 'text-amber-400'}>
                    {registrationProbe.live ? t('diagLive') : t('diagNotLive')}
                  </span>
                </p>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-foreground">{t('credsTitle')}</CardTitle>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <p className="text-xs text-muted-foreground leading-relaxed">
                {t.rich('pinHint', {
                  b: (chunks) => (
                    <strong className="text-muted-foreground">{chunks}</strong>
                  ),
                })}
              </p>
            </div>
              ...
              <AccordionItem className="border-border">
                ...
                <AccordionContent className="text-muted-foreground">
                  <ol className="list-decimal list-inside space-y-1 text-sm">
                    <li>
                      {t.rich('setup.step1Item1', {
                        host: (chunks) => <span className="text-primary">{chunks}</span>,
                      })}
                    </li>
              ...
              <AccordionItem className="border-border">
                ...
                <AccordionContent className="text-muted-foreground">
                  <ol className="list-decimal list-inside space-y-1 text-sm">
                    ...
                    <li>{t.rich('setup.step3Item2', { b: (c) => <strong className="text-foreground">{c}</strong> })}</li>
                    <li>{t.rich('setup.step3Item3', { b: (c) => <strong className="text-foreground">{c}</strong> })}</li>
                    <li>{t.rich('setup.step3Item4', { b: (c) => <strong className="text-foreground">{c}</strong> })}</li>
                  </ol>
                </AccordionContent>
              </AccordionItem>
              ...
              <AccordionItem className="border-border">
                ...
                <AccordionContent className="text-muted-foreground">
                  <ol className="list-decimal list-inside space-y-1 text-sm">
                    ...
                    <li>{t.rich('setup.step4Item3', { b: (c) => <strong className="text-foreground">{c}</strong> })}</li>
                    <li>{t.rich('setup.step4Item4', { b: (c) => <strong className="text-foreground">{c}</strong> })}</li>
