...
// are looked up via the `settings.roles` catalogue at render time.
          ...
          return (
            <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-muted-foreground">
              <span className="inline-flex items-center gap-1.5">
                ...
                {t('online', { count: counts.online })}
              </span>
              <span className="inline-flex items-center gap-1.5">
                <PresenceDot status="away" />
                {t('away', { count: counts.away })}
              </span>
              <span className="inline-flex items-center gap-1.5">
                <PresenceDot status="offline" />
                {t('offline', { count: counts.offline })}
              </span>
                    ...
                    >
                      <div className="min-w-0 flex-1">
                        ...
                        <p className="mt-0.5 text-xs text-muted-foreground">
                          {t('created', { date: fmtDate(inv.created_at) })} · {expiresLabel(inv.expires_at)}
                        </p>
                      </div>
              ...
              {t.rich('removeDesc', {
                name: () => (
                  <span className="font-medium text-muted-foreground">
                    {removingMember?.full_name || t('removeTeammateFallback')}
                  </span>
                ),
              })}
            </DialogDescription>
