...
export function ContactDetailView({
  ...
  async function addNote() {
    ...
    if (error) {
      toast.error(t('toasts.addNoteFailed'));
    } else {
      setNewNote('');
      fetchNotes();
  ...
  async function deleteNote(noteId: string) {
    ...
    if (error) {
      toast.error(t('toasts.deleteNoteFailed'));
    } else {
  ...
  async function saveCustomFields() {
    ...
    try {
      ...
      toast.success(t('toasts.customSaved'));
    } catch {
      toast.error(t('toasts.customSaveFailed'));
    }
  ...
  async function handleSendTemplate(
    ...
    try {
      ...
      toast.success(t('toasts.templateSent', { name: template.name }));
    } catch (err) {
      const reason = err instanceof Error ? err.message : t('toasts.networkError');
      toast.error(t('toasts.sendTemplateFailed', { reason }));
    } finally {
                    ...
                    notes.map((note) => (
                      ...
                      >
                        ...
                        <p className="text-xs text-muted-foreground mt-1.5">
                          {fmt.date(new Date(note.created_at), {
                            month: 'short',
                            day: 'numeric',
                            year: 'numeric',
                            hour: '2-digit',
                    ...
                    {customFields.map((field) => (
                      <div key={field.id} className="space-y-1.5">
                        ...
                        <Input
                          ...
                          placeholder={t('customFieldPlaceholder', { name: field.field_name })}
