...
// Labels resolved at render time via `step3.contactFields.<value>`.
const contactFields = [
  { value: 'name' },
  { value: 'phone' },
  { value: 'email' },
...
export function Step3Personalize({
  ...
  const previewLabel = firstContact
    ...
    : t('step3.preview.sampleData');

  const mediaTypeLabel = mediaHeaderType
    ? t(`step3.header.types.${mediaHeaderType}`)
    : '';
    ...
    <div className="space-y-6">
      ...
      {mediaHeaderType && (
        <div className="rounded-xl border border-border bg-card/50 p-4">
          <div className="mb-3 flex items-center gap-2">
            ...
            <p className="text-sm font-medium text-foreground">{t('step3.header.title')}</p>
          ...
          <label className="mb-1.5 block text-xs font-medium text-muted-foreground">
            {t('step3.header.urlLabel')}
          </label>
          <Input
            type="url"
            value={headerMediaUrl}
            onChange={(e) => onHeaderMediaUrlChange(e.target.value)}
            placeholder={t('step3.header.urlPlaceholder', {
              ext:
          ...
          <p className="mt-1.5 text-xs text-muted-foreground">
            {t('step3.header.description', { type: mediaTypeLabel })}
          </p>
          {mediaHeaderType === 'image' &&
          ...
          {headerMediaError && (
            <p className="mt-1.5 text-xs text-amber-300">
              {headerMediaError === 'missing'
                ? t('step3.header.errorMissing')
                : t('step3.header.errorInvalid')}
            </p>
          )}
        </div>
      )}
            ...
            return (
              ...
              >
                ...
                <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                  <div>
                    ...
                    >
                      ...
                      <SelectContent className="border-border bg-popover">
                        <SelectItem value="static">{t('step3.mapping.static')}</SelectItem>
                        <SelectItem value="field">{t('step3.mapping.field')}</SelectItem>
                        <SelectItem value="custom_field">
                          {t('step3.mapping.customField')}
                        </SelectItem>
                      </SelectContent>
                    </Select>
                  </div>
                  ...
                  <div>
                    <label className="mb-1.5 block text-xs font-medium text-muted-foreground">
                      {mapping.type === 'static' ? t('step3.valueLabel') : t('step3.fieldLabel')}
                    </label>
                    {mapping.type === 'static' ? (
                      <Input
                        value={mapping.value}
                      ...
                      >
                        <SelectTrigger className="w-full border-border bg-muted text-foreground">
                          <SelectValue placeholder={t('step3.selectField')} />
                        </SelectTrigger>
                        <SelectContent className="border-border bg-popover">
                          {contactFields.map((field) => (
                            <SelectItem key={field.value} value={field.value}>
                              {t(`step3.contactFields.${field.value}`)}
                            </SelectItem>
                          ))}
                        </SelectContent>
                      </Select>
      ...
      <div className="rounded-xl border border-border bg-card/50 p-4">
        <div className="mb-3 flex items-center gap-2">
          ...
          <p className="text-sm font-medium text-foreground">{t('step3.preview.title')}</p>
      ...
      {unmappedKeys.length > 0 && (
        <div className="rounded-md border border-amber-500/30 bg-amber-500/10 px-3 py-2 text-xs text-amber-300">
          {t('step3.unmapped.prefix')}
          <span className="font-mono font-semibold">
            {unmappedKeys.join(', ')}
          </span>
          {t('step3.unmapped.suffix')}
        </div>
      )}
