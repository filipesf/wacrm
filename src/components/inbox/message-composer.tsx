...
export function MessageComposer({
  ...
  const finalizeRecording = useCallback(
    async (bytes: Uint8Array) => {
      ...
      if (file.size > MEDIA_MAX_BYTES_BY_KIND.audio) {
        toast.error(t("composer.recordingTooLong"));
        return;
      }
      setBusy(true);
      try {
  ...
  const startRecording = useCallback(async () => {
    ...
    try {
      ...
      toast.error(t("composer.microphoneDenied"));
    }
      ...
      {sessionExpired && (
        <div className="mb-2 flex items-center justify-between rounded-lg bg-amber-500/10 px-3 py-2">
          <p className="text-xs text-amber-400">
            {t("composer.sessionExpiredBanner")}
          </p>
          <Button
            variant="ghost"
            size="sm"
      ...
      {draft ? (
        ...
        <div className="flex items-center gap-3 rounded-xl border border-border bg-muted px-4 py-2.5">
          ...
          <Button
            ...
            title={t("composer.stopAndAttach")}
          >
        ...
        <div className="flex items-end gap-2">
          ...
          <GatedButton
            ...
            size="sm"
            canAct={!readOnly}
            gateReason={t('gate.send')}
            title={readOnly ? undefined : t("composer.sendTemplate")}
          ...
          <textarea
            ...
            title={readOnly ? t("composer.attachReadOnly") : undefined}
            className={cn(
